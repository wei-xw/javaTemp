(function() {
	...
	render = function(params) {
		...
		createChart = function(inputOption,inputData, width, height) {
			var counter = 0;
			...
			function loadScript(varStr, codeStr,dataStr) {
				var script = $('<script>').attr('type', 'text/javascript');
				script.html('var data='+dataStr+';'+codeStr);
			...
			if (inputOption != 'undefined') {
				// parse the options
				loadScript('option&data', inputOption,inputData);
		...
		page.open("about:blank", function(status) {
			...
			// create the chart
			page.evaluate(createChart, params.options,params.data, width, height);
	...
	var params = parseParams();
		...
		console.log("ERROR: No data or infile found.");
		usage();
		phantom.exit();
	}
	// validate the params
